...
cap = cv2.VideoCapture('assets\store_video3.mp4')
...
#Initialize Dictionary {(Tracked Objects,Ids)}
tracking_objects = {}
...
while True:
    ...
    region_A = [(542,67), (298,333), (430,850), (606,465)]
    for area in [region_A]:
        cv2.polylines(frame, [np.array(area, np.int32)], True, (15, 220, 10), 8)
    
    region_B =  [(1198,67),(1469,277), (1318,870), (1180,450)]
    for area in [region_B]:
        cv2.polylines(frame, [np.array(area, np.int32)], True, (15, 220, 10), 8)
    
    region_C = [(1330,950),(1501,299),(1920,686),(1920,1075), (1376,1075)]
    for area in [region_C]:
        cv2.polylines(frame, [np.array(area, np.int32)], True, (15, 220, 10), 8)

    #Initialize Object Detection
    (class_ids, scores, boxes) = model.detect(frame, nmsThreshold=0.3, confThreshold=0.5)
    ...
    for object_id, pt in tracking_objects.items():
        ...
        if inside_region_A == 1:
            ...
            total_person = str(len(region_A_ids)+len(region_B_ids)+len(region_C_ids))
    ...
    print("-------------------------")
    print("ID Tracked in Region 3:", str(region_C_ids))
    print("Total Person in Region 3:", len(region_C_ids))
    print(str(region_C_ids))

    print("\nTotal All Customers in All Regions: ",len(region_A_ids)+len(region_B_ids)+len(region_C_ids))
